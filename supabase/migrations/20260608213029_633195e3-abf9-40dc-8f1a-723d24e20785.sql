...
ALTER TABLE public.notification_templates
  ADD COLUMN IF NOT EXISTS trigger_after_days INTEGER,
  ADD COLUMN IF NOT EXISTS repeat_every_days INTEGER;

-- Set default trigger values for existing templates
UPDATE public.notification_templates SET trigger_after_days = -5 WHERE template_key = 'before_5' AND trigger_after_days IS NULL;
UPDATE public.notification_templates SET trigger_after_days = 1  WHERE template_key = 'after_1'  AND trigger_after_days IS NULL;
UPDATE public.notification_templates SET trigger_after_days = 7  WHERE template_key = 'after_7'  AND trigger_after_days IS NULL;

-- Add new "overdue_monthly" template for every existing org
INSERT INTO public.notification_templates
  (organization_id, template_key, label, sms_content, email_content, trigger_after_days, repeat_every_days, sms_enabled, email_enabled)
SELECT
  o.id,
  'overdue_monthly',
  'Loyer impayé > 1 mois',
  'Bonjour {{nom}}, votre loyer de {{montant}} FCFA est impayé depuis plus d''un mois (échéance {{date_echeance}}). Merci de régulariser au plus vite.',
  E'Bonjour {{nom}},\n\nVotre loyer de {{montant}} FCFA dû le {{date_echeance}} est impayé depuis plus d''un mois.\n\nMerci de régulariser au plus vite.\n\nCordialement',
  30,
  7,
  true,
  false
...
WHERE NOT EXISTS (
  ...
  WHERE t.organization_id = o.id AND t.template_key = 'overdue_monthly'
);

-- Update handle_new_user to seed the new template for future orgs
...
BEGIN
  ...
  INSERT INTO public.notification_templates (organization_id, template_key, label, sms_content, email_content, trigger_after_days, repeat_every_days) VALUES
    ...
    (new_org_id, 'overdue_monthly', 'Loyer impayé > 1 mois', 'Bonjour {{nom}}, votre loyer de {{montant}} FCFA est impayé depuis plus d''un mois (échéance {{date_echeance}}). Merci de régulariser au plus vite.', E'Bonjour {{nom}},\n\nVotre loyer de {{montant}} FCFA dû le {{date_echeance}} est impayé depuis plus d''un mois.\n\nMerci de régulariser au plus vite.\n\nCordialement', 30, 7);
